...
struct MessageTextView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if appEnvironment.isLinkPreviewEnabled, let metadata = linkMetadata {
                LinkPreviewView(metadata: metadata)
            }
        }
        .onAppear {
            if appEnvironment.isLinkPreviewEnabled, let text = text, let url = FormValidator.extractFirstURL(from: text) {
                LinkPreviewProvider.shared.fetchMetadata(for: url) { metadata in
...
//            .font: UIFont.systemFont(ofSize: 32) // Установите нужный шрифт и размер
//        ]
        let attributes: [NSAttributedString.Key: Any] = [
                .font: font
            ]
        return NSAttributedString(string: processedText)
    }
